...
for i, filename in enumerate(args.files):
    ...
    datasets[-1].extend([b["memory_usage_byte"][0] for b in results])
    print(datasets[-1])
...
rects1 = ax.bar(x - width/2, datasets[0], width, label='Baseline', color='orange')

# Plot the second set of bars, shifted right by half the width
rects2 = ax.bar(x + width/2, datasets[1], width, alpha=0.5, label='Instrumented', color='red')

# Add labels, title, and legend
ax.set_xticks(x, categories)
ax.grid(visible=True, axis="y")
# ax.set_ylim(0, max(max(datasets[0]), max(datasets[1])) * 1.1)  # Set y-axis limit for better visibility
ax.set_title('Max Memory Usage: Comparison Between Baseline and Instrumented Binaries')
